...
async function transfer(sender, senderPublicKey, senderPrivateKey, receiver, amount) {
...
async function transferCreate(sender, senderPublicKey, senderPrivateKey, receiver, amount) {
  const transaction = Pact.builder
    .execution(
      // pact expression
...
async function createAccount(sender, senderPublicKey, senderPrivateKey, receiver) {
  const transaction = Pact.builder
...
async function insertCandidates(sender, senderPublicKey, candidates) {
  const transaction = Pact.builder
    .execution(
      // pact expression
...
const main = async () => {
  ...
  await transferCreate(FUNDING_ACCOUNT, FUNDING_ACCOUNT_PUBLIC_KEY, FUNDING_ACCOUNT_PRIVATE_KEY, ADMIN_ACCOUNT, {
    decimal: '20.00',
  });
  ...
  await transfer(FUNDING_ACCOUNT, FUNDING_ACCOUNT_PUBLIC_KEY, FUNDING_ACCOUNT_PRIVATE_KEY, GAS_STATION_ACCOUNT, {
    decimal: '20.00',
  });
  // Add candidates
